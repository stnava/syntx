...
def main():
    print("Loading 3D mbhard benchmark data...")
    ...
    # Crop to save time on empty space
    mask_f = ants.iMath(ants.get_mask(fi), "MD", 10)
    fi = ants.crop_image(fi, mask_f)
    fl = ants.crop_image(fl, mask_f)

    mask_m = ants.iMath(ants.get_mask(mi), "MD", 10)
    mi = ants.crop_image(mi, mask_m)
    ml = ants.crop_image(ml, mask_m)
    ...
    with open('benchmark_3d_mbhard_results.json', 'w') as f:
