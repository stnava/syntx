...
def jacobian_determinant(disp, spacing=None, ref_image=None):
    ...
    if dim == 3 and arr.ndim == 4:
        # 3D: arr shape (Z, Y, X, 3) from ants.image_read().numpy()
        # ITK component order: comp 0 = dx (axis 2), comp 1 = dy (axis 1), comp 2 = dz (axis 0)
        # Spatial axes: axis 0 = Z (spacing sp[2]), axis 1 = Y (spacing sp[1]), axis 2 = X (spacing sp[0])
        comp_map = [2, 1, 0]
        sp_axis = [sp[2], sp[1], sp[0]]  # spacing per axis: [sp_z, sp_y, sp_x]

        J = np.zeros((*arr.shape[:3], 3, 3), dtype=np.float32)
        for i in range(3):
            for j in range(3):
                deriv = np.gradient(arr[..., comp_map[i]], axis=j) / sp_axis[j]
                if i == j:
                    J[..., i, j] = 1.0 + deriv
                else:
                    J[..., i, j] = deriv
        ...
        # ITK component order: comp 0 = dx (axis 1), comp 1 = dy (axis 0)
        # Spatial axes: axis 0 = Y (spacing sp[1]), axis 1 = X (spacing sp[0])
        sp_axis = [sp[1], sp[0]]  # spacing per axis: [sp_y, sp_x]

        du_y_d0 = np.gradient(arr[..., 1], axis=0) / sp_axis[0]  # d(dy)/dY
        du_y_d1 = np.gradient(arr[..., 1], axis=1) / sp_axis[1]  # d(dy)/dX
        du_x_d0 = np.gradient(arr[..., 0], axis=0) / sp_axis[0]  # d(dx)/dY
        du_x_d1 = np.gradient(arr[..., 0], axis=1) / sp_axis[1]  # d(dx)/dX

        return (1.0 + du_y_d0) * (1.0 + du_x_d1) - du_y_d1 * du_x_d0
