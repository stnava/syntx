"""
image_compare.py — Unified Multi-Metric Image Similarity & Distance Evaluation Framework
========================================================================================

`image_compare` provides a unified, cross-dimensional (2D and 3D) evaluation API supporting
over 24 classical, gradient-based, mutual information, and deep feature metrics.

Standardized Evaluation Protocol (GEMINI.md Rule 5)
---------------------------------------------------
1. Standardized Returns (Lower is Better): All metrics evaluated through `image_compare`
   return scores where a lower value strictly indicates higher similarity. For metrics
   traditionally maximized (e.g. PSNR, NCC, SSIM), the inverted or negative value is returned
   (e.g., `-PSNR`, `1 - NCC`, `1 - SSIM`).
2. 2D and 3D Dimensionality: All metrics natively support 2D and 3D inputs. When using 2D-native
   deep feature backbones (VGG19, DINOv2), a triplanar ensemble or feature volume reconstruction
   is used to support 3D images.
"""
...
def to_torch(x) -> torch.Tensor:
    """
    Converts diverse image data types into a PyTorch tensor.

    Supports ANTsImage, PyTorch Tensors, JAX Arrays (`Array`, `DeviceArray`), and NumPy arrays.

    Parameters
    ----------
    x : ANTsImage, torch.Tensor, jax.Array, or np.ndarray
        Input image object.

    Returns
    -------
    torch.Tensor
        Converted PyTorch tensor with detached memory reference.
    """
...
def standardize_tensor(t: torch.Tensor):
    """
    Squeezes redundant singleton batch and channel dimensions from input image tensors.

    Parameters
    ----------
    t : torch.Tensor
        Input image tensor of arbitrary shape (e.g. `(1, 1, H, W)` or `(D, H, W)`).

    Returns
    -------
    t_std : torch.Tensor
        Standardized spatial tensor of shape `(H, W)` for 2D or `(D, H, W)` for 3D.
    ndim : int
...
def compute_histograms(a_np: np.ndarray, b_np: np.ndarray, bins: int = 32):
    """
    Computes marginal and joint Shannon entropies from 2D joint intensity histograms.

    Parameters
    ----------
    a_np : np.ndarray
        Flattened NumPy array for image A.
    b_np : np.ndarray
        Flattened NumPy array for image B.
    bins : int, default=32
    ...
    -------
    h_a : float
        Marginal entropy of image A in bits.
    h_b : float
        Marginal entropy of image B in bits.
    h_ab : float
        Joint entropy of image pair (A, B) in bits.
    """
...
def ssim_torch(img1: torch.Tensor, img2: torch.Tensor, window_size: int = 11, size_average: bool = True):
    """
    PyTorch Implementation of 2D and 3D Structural Similarity (SSIM).

    Parameters
    ----------
    img1 : torch.Tensor
        Image tensor 1 of shape `(1, 1, H, W)` or `(1, 1, D, H, W)`.
    img2 : torch.Tensor
        Image tensor 2 of matching shape.
    window_size : int, default=11
        Average pooling window size.
    size_average : bool, default=True
        If True, returns scalar mean SSIM value; otherwise returns full SSIM map.

    Returns
    -------
    torch.Tensor
        Scalar mean SSIM or spatial SSIM map.
    """
    dim = img1.ndim - 2
    if dim == 2:
...
def ms_ssim_torch(img1: torch.Tensor, img2: torch.Tensor, window_size: int = 11):
    """
    PyTorch Implementation of Multi-Scale SSIM supporting 2D and 3D image inputs.
    ...
    img2 : torch.Tensor
        Image tensor 2 of matching shape.
    window_size : int, default=11
        Base pooling window size.

    Returns
    -------
    torch.Tensor
        Scalar Multi-Scale SSIM score.
    """
    dim = img1.ndim - 2
    if dim == 2:
        pool_fn = F.avg_pool2d
...
def compute_gradients_torch(img: torch.Tensor) -> torch.Tensor:
    """
    Computes spatial central difference gradients of a 2D or 3D image tensor.

    Parameters
    ----------
    img : torch.Tensor
        Image tensor of shape `(1, 1, H, W)` or `(1, 1, D, H, W)`.

    Returns
    -------
    torch.Tensor
        Gradient vector tensor of shape `(1, 2, H, W)` for 2D `[dy, dx]` or `(1, 3, D, H, W)` for 3D `[dz, dy, dx]`.
    """
    dim = img.ndim - 2
    if dim == 2:
...
def compute_reconstructed_loss(extractor, a_3d: torch.Tensor, b_3d: torch.Tensor, loss_type: str = 'l1') -> float:
    """
    Calculates reconstructed triplanar feature loss for 2D networks processing 3D volumes.

    Parameters
    ----------
    extractor : FeatureExtractor
        2D Feature extractor instance.
    a_3d : torch.Tensor
        First 3D image volume.
    b_3d : torch.Tensor
        Second 3D image volume.
    loss_type : str, default='l1'
        Feature distance loss type ('l1', 'l2', 'cos').

    Returns
    -------
    float
        Average distance score across reconstructed Axial, Coronal, and Sagittal feature volumes.
    """
    D, H, W = a_3d.shape[2:]
    B = a_3d.shape[0]
...
def compute_direct_loss(extractor, a_nd: torch.Tensor, b_nd: torch.Tensor, loss_type: str = 'l1') -> float:
    """
    Calculates direct feature distance loss for 2D inputs or native 3D feature extractors.
...
def get_cached_extractor(model_name: str, dim: int, layer: int, device):
    """
    Retrieves or instantiates and caches a deep feature extractor module.

    Parameters
    ----------
    model_name : str
        Extractor network family ('vgg', 'dino', 'resnet', 'swin').
    dim : int
        Spatial dimensionality (2 or 3).
    layer : int
        Target feature layer index.
    ...
    -------
    FeatureExtractor
        Cached feature extractor module on `device`.
    """
...
def deep_feature_loss(a_unsq: torch.Tensor, b_unsq: torch.Tensor, extractor, l: int, mtype: str, dim: int) -> float:
    """
    Orchestrates deep feature loss evaluation across LNCC, L1, L2, and Cosine similarity metrics.

    Parameters
    ----------
    a_unsq : torch.Tensor
        4D/5D input image tensor A.
    b_unsq : torch.Tensor
        4D/5D input image tensor B.
    extractor : FeatureExtractor
        Feature extractor instance.
    l : int
        Target feature layer index.
    mtype : str
        Loss type ('lncc', 'cc', 'l1', 'l2', 'cos').
    dim : int
        Spatial dimensionality (2 or 3).
    ...
    -------
    float
        Computed deep feature distance score.
    """
    if mtype in ['lncc', 'cc']:
        if dim == 3:
