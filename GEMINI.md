...
* **ITK Gaussian Kernel Parity (`kernel_type='gaussian'`)**: `kernel_type='gaussian'` (ITK-compatible truncated sampled Gaussian with radius $\lfloor 3\sigma + 0.5 \rfloor$) outperforms the discrete Bessel kernel (`kernel_type='bessel'`) on SyN registration tasks (yielding $+0.68\%$ Symmetric Dice gain and reducing grid folding by $>50\%$). Use `kernel_type='gaussian'` as the recommended standard for ITK parity comparisons.
