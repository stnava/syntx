...
* **PyTorch Affine Solver Defaults (`robust_affine(mode='pytorch')`, 2026-09-14)**:
  - Schedule-driven single optimisation path (`_default_affine_schedule`: level 4 rigid 50 it → level 2 affine 50 it + best-of-`n_starts` selection → level 1 affine 30 it, Adam + cosine), seeded coarse-level candidate search (identity at CoM, ±4/8/12° single-axis rotations, optional `initial_transform` as a re-centred candidate).
  - Point-sampled objective: per level a fixed, seeded random subset of 100k fixed-domain voxels (`n_sample_points`); Mattes MI with 64 bins, fixed bounds (0, 1), whole-domain mask. mbhard: Dice 0.3265 in ~6.5 s on MPS vs ANTs 0.3255 in ~43 s; bitwise reproducible per device. Longer schedules do not improve Dice (objective, not convergence, was the gap).
