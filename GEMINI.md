...
* **PyTorch Affine Solver Defaults & Presets (`robust_affine(mode='pytorch')`, 2026-09-14)**:
  - One schedule-driven optimisation path (`_AffinePath`: `A = R(ω)·B·diag(eˢ)·Shear`), seeded coarse-level candidates (identity at CoM, ±4/8/12° single-axis rotations, optional `initial_transform` re-centred as a candidate; a landmark affine from `syntx.landmarks` is a valid seed), best-of-`n_starts` (2) selection by exact MI.
