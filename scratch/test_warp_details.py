...
# Force identity direction cosines on both images to isolate coordinate systems
fi_low.set_direction(np.eye(3))
mi_low.set_direction(np.eye(3))

# 1. Run ANTs Affine Only to get a common starting transform
print("--- Running ANTs Affine (Identity Direction) ---")
reg_aff = ants.registration(
    fixed=fi_low, moving=mi_low, type_of_transform='Affine'
)
aff_tx = reg_aff['fwdtransforms'][0]

# 2. Run ANTs SyN for 1 iteration starting from aff_tx
print("--- Running ANTs SyN starting from Affine (Identity Direction) ---")
reg_ants = ants.registration(
    ...
    reg_iterations=[1, 0, 0],
    grad_step=0.1 # Pass grad_step=0.1 to match JAX
)
warp_ants = ants.image_read(reg_ants['fwdtransforms'][0])
warp_ants_arr = warp_ants.numpy()

# 3. Run JAX SyN for 1 iteration starting from aff_tx
print("--- Running JAX SyN starting from Affine (Identity Direction) ---")
reg_jax = registration(
    fixed=fi_low, moving=mi_low, type_of_transform='SyN',
    backend='jax', syn_metric='lncc', syn_sampling=4,
    initial_transform=aff_tx,
    levels=[4, 2, 1], # Match 3 levels
    affine_iterations=[0, 0, 0], # Disable JAX affine stage
    reg_iterations=[1, 0, 0], # 1 iteration at Level 0
    grad_step=0.1,
    flow_sigma=3.0,
    verbose=False
)
...
print("\nANTs Warp statistics (Identity Direction):")
for i in range(3):
    arr = warp_ants_arr[..., i]
    print(f"Component {i}: min={arr.min():.6f}, max={arr.max():.6f}, mean={arr.mean():.6f}, std={arr.std():.6f}")

print("\nJAX Warp statistics (Identity Direction):")
for i in range(3):
    arr = warp_jax_arr[..., i]
    print(f"Component {i}: min={arr.min():.6f}, max={arr.max():.6f}, mean={arr.mean():.6f}, std={arr.std():.6f}")

print("\nFull 3x3 correlation matrix under IDENTITY Direction:")
for i in range(3):
    row_corrs = []
    for j in range(3):
