...
for step in range(30):
    ...
    with torch.no_grad():
        ...
        # Greedy composition: u_new = u - delta (standard gradient descent)
...
print("\nFinished optimization. Evaluating different compositions...")

with torch.no_grad():
    # 1. Half warp (original implementation): warp_l2r
    fwd_file_half = tempfile.NamedTemporaryFile(suffix='_half.nii.gz', delete=False).name
    fwd_img_half = ants.from_numpy(warp_l2r[0].numpy(), origin=fi.origin, spacing=fi.spacing, direction=fi.direction, has_components=True)
    ants.image_write(fwd_img_half, fwd_file_half)
    warped_half = ants.apply_transforms(fi, mi_affine, [fwd_file_half])
    mi_half = ants.image_mutual_information(fi, warped_half)
    print(f"Original Half Warp (warp_l2r) | ANTs MI: {mi_half:.6f}")
    os.remove(fwd_file_half)
    
    # 2. Composed Warp: warp_l2r_inv + warp_r2l(X_phys + warp_l2r_inv)
    # This represents the mapping from fixed space to moving space:
    # x_fixed -> x_mid -> x_moving
    # Since warp_l2r_inv maps fixed to mid, and warp_r2l maps mid to moving:
    # warp_fwd = warp_l2r_inv + warp_r2l(X_phys + warp_l2r_inv)
    coords_norm_inv = physical_to_normalized_torch(X_phys + warp_l2r_inv, spatial, fi.spacing, fi.origin, fi.direction)
    ...
    fwd_img_comp = ants.from_numpy(warp_fwd[0].numpy(), origin=fi.origin, spacing=fi.spacing, direction=fi.direction, has_components=True)
    ...
    print(f"Composed Warp (warp_l2r_inv o warp_r2l) | ANTs MI: {mi_comp:.6f}")
