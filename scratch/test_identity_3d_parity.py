...
# Create synthetic 3D images with identity physical space
print("Generating synthetic 3D images...")
shape = (64, 64, 64)
fi_np = np.zeros(shape, dtype=np.float32)
mi_np = np.zeros(shape, dtype=np.float32)

# Create a sphere in fixed image
x, y, z = np.ogrid[:64, :64, :64]
mask_fi = (x - 32)**2 + (y - 32)**2 + (z - 32)**2 <= 15**2
fi_np[mask_fi] = 1.0

# Create a shifted sphere in moving image
...
moving_seg = ants.threshold_image(mi, 0.5)
...
dice_init, mi_init = get_metrics(mi)
print(f"Initial Baseline | Dice: {dice_init:.6f} | MI: {mi_init:.6f}")

# 1. Run ANTs SyN (SyNOnly, 15 iterations)
print("\nRunning ANTs SyN...")
...
# 2. Run Syntx PyTorch SyN (SyNOnly, 15 iterations)
...
# 3. Run Syntx JAX SyN (SyNOnly, 15 iterations)
