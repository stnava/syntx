...
# PyTorch
def local_ncc_loss_no_mask_py(I, J):
    def box_filter(x):
    ...
    cc = IJ_cov / (torch.sqrt(safe_I_var * safe_J_var) + 1e-8)
    return -torch.mean(cc)

x_py = torch.tensor(x_np, dtype=torch.float32).requires_grad_(True)
y_py = torch.tensor(y_np, dtype=torch.float32)
loss_py = local_ncc_loss_no_mask_py(y_py, x_py)
loss_py.backward()
g_py = x_py.grad.numpy()

# JAX
def local_ncc_loss_no_mask_jax(I, J):
    I_mean = box_filter_jax(I, 9)
    J_mean = box_filter_jax(J, 9)
    I_var = box_filter_jax(I**2, 9) - I_mean**2
    J_var = box_filter_jax(J**2, 9) - J_mean**2
    IJ_cov = box_filter_jax(I*J, 9) - I_mean * J_mean
    safe_I_var = jnp.maximum(I_var, 1e-8)
    safe_J_var = jnp.maximum(J_var, 1e-8)
    cc = IJ_cov / (jnp.sqrt(safe_I_var * safe_J_var) + 1e-8)
    return -jnp.mean(cc)

x_jax = jnp.array(x_np)
y_jax = jnp.array(y_np)
loss_fn = lambda x: local_ncc_loss_no_mask_jax(y_jax, x)
...
print("Ratio of PyTorch to JAX grad max:", float(np.abs(g_py).max() / np.abs(g_jax).max()))
