...
print("\n--- 1. JAX SyN (levels=[8, 4, 2, 1], reg_iterations=[100, 70, 50, 20]) ---")
t0 = time.time()
reg_jax_1 = registration(
...
t1 = time.time() - t0
dice_jax_1 = compute_mean_dice(fl, ml, reg_jax_1['fwdtransforms'])
print(f"JAX 4-level DICE: {dice_jax_1:.4f} ({t1:.2f}s)")

# Clean up
for path in reg_jax_1['fwdtransforms'] + reg_jax_1['invtransforms']:
    if os.path.exists(path):
        os.remove(path)

print("\n--- 2. JAX SyN (levels=[4, 2, 1], reg_iterations=[100, 70, 50]) ---")
t0 = time.time()
reg_jax_2 = registration(
    fixed=fi_low, moving=mi_low, type_of_transform='SyN',
    backend='jax', syn_metric='lncc', syn_sampling=4,
    levels=[4, 2, 1],
...
t2 = time.time() - t0
dice_jax_2 = compute_mean_dice(fl, ml, reg_jax_2['fwdtransforms'])
print(f"JAX 3-level DICE: {dice_jax_2:.4f} ({t2:.2f}s)")

# Clean up
for path in reg_jax_2['fwdtransforms'] + reg_jax_2['invtransforms']:
    if os.path.exists(path):
        os.remove(path)
