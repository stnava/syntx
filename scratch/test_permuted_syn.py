...
# Permute to ZYX/YX order
perm = [0, 1] + list(range(dim + 1, 1, -1))
I_tensor_zyx = I_tensor.permute(perm)
J_tensor_zyx = J_tensor.permute(perm)

spatial = J_tensor_zyx.shape[2:]
...
for step in range(30):
    ...
    I_mid, J_mid, grad_I_mid_sampled, grad_J_mid_sampled = prepare_mid_images_and_gradients_torch(
        warp_l2r, warp_r2l, warp_l2r_inv, warp_r2l_inv, I_tensor_zyx, J_tensor_zyx,
        X_phys,
...
print("\nFinished optimization. Evaluating composed warp...")
...
with torch.no_grad():
    ...
    # Export with [..., ::-1] because the internal field is ZYX/YX order!
    ...
    print(f"Composed Warp (correctly permuted) | ANTs MI: {mi_comp:.6f}")
    ...
    print(f"Composed Warp | Otsu Dice: {dice:.4f}")
