...
py_img = reg_syn_py['warpedmovout'].numpy()
jax_img = reg_syn_jax['warpedmovout'].numpy()

max_diff = np.abs(py_img - jax_img).max()
mean_diff = np.abs(py_img - jax_img).mean()
print(f"\nMax difference between PyTorch and JAX warped images: {max_diff:.6f}")
print(f"Mean difference between PyTorch and JAX warped images: {mean_diff:.6f}")
