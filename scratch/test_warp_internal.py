...
X_phys = syntx.syn.get_physical_grid_torch(fi.shape, fi.spacing, fi.origin, fi.direction, device=total_fwd.device, dtype=total_fwd.dtype)

# total_fwd maps fixed to moving_curr
Z = X_phys + total_fwd

# moving_curr is the mi_affine transformed by model.affine
# we can just use mi_affine directly since model.affine is almost identity, 
# but let's do it exactly:
T_grid = model.affine.get_matrix().detach()
M_phys, t_phys = syntx.syn.grid_to_physical_affine(T_grid, fi, mi_affine)
y_phys = Z @ M_phys.t() + t_phys

y_norm = syntx.syn.physical_to_normalized_torch(y_phys, mi_affine.shape, mi_affine.spacing, mi_affine.origin, mi_affine.direction)
