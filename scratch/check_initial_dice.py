...
# 1. Identity in voxel space (no header alignment)
# (Skipped due to shape mismatch)

# 2. Physical Identity (align using headers, i.e., empty transform list)
dice_phys = compute_mean_dice(fl, ml, [])
print(f"Physical Identity DICE (header alignment only): {dice_phys:.4f}")
