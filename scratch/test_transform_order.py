...
# Test current order
transforms_current = reg_jax['fwdtransforms']
print(f"Current transforms list: {transforms_current}")
dice_current = compute_mean_dice(fl, ml, transforms_current)
print(f"DICE with current order: {dice_current:.4f}")

# Test reversed order
transforms_reversed = list(reversed(transforms_current))
print(f"Reversed transforms list: {transforms_reversed}")
dice_reversed = compute_mean_dice(fl, ml, transforms_reversed)
print(f"DICE with reversed order: {dice_reversed:.4f}")
