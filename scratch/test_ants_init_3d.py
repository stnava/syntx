...
print("\nRunning Syntx Affine initialized with ANTs Affine (0 epochs)...")
reg_syn = syntx.syn(
    fixed=fi, moving=mi, type_of_transform='Affine', backend='pytorch',
    initial_transform=reg_ants['fwdtransforms'],
    affine_iterations=[0]
)

warped_syn_ml = ants.apply_transforms(fi, ml, reg_syn['fwdtransforms'], interpolator='nearestNeighbor')
overlap_syn = ants.label_overlap_measures(fl, warped_syn_ml)
overlap_syn = overlap_syn[(overlap_syn['Label'] != 0) & (overlap_syn['Label'] != 'All')]
print("Syntx Affine 3D DKT Dice:", overlap_syn['TotalOrTargetOverlap'].mean())
