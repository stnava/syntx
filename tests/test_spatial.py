...
class TestJacobianDeterminant:
    ...
    def test_jacobian_4d_tensor_ref_image_none(self):
        """4D batched tensor (1, 32, 32, 2) with ref_image=None should not crash."""
        disp_4d = torch.zeros(1, 32, 32, 2)
        detJ = jacobian_determinant(disp_4d, ref_image=None)
        assert isinstance(detJ, np.ndarray)
        assert detJ.shape == (32, 32)
        np.testing.assert_array_almost_equal(detJ, np.ones((32, 32)))
