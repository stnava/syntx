...
The variational energy functional balances structural image similarity at the midpoint domain with spatial smoothness on the underlying velocity fields [@avants2008symmetric; @vercauteren2009diffeomorphic]:
...
The variational $L^2$ functional derivative with respect to the moving image intensity $I_M$ is [@avants2008symmetric; @paszke2019pytorch]:
...
Spatial 3D rotations form the special orthogonal Lie group $\text{SO}(3) = \{ R \in \mathbb{R}^{3 \times 3} : R^T R = I, \det(R) = 1 \}$. We parameterize rotations via the Lie algebra $\mathfrak{so}(3)$ of skew-symmetric matrices using the angular velocity vector $\boldsymbol{\omega} = (\omega_x, \omega_y, \omega_z)^T \in \mathbb{R}^3$ [@chirikjian2011stochastic].
...
Standard gradient descent on the affine Lie group $\text{Aff}(3)$ readily converges to local suboptimal basins when subject brains exhibit angular pitch or yaw tilt relative to standard coordinate space [@jenkinson2001global].

We formulate a deterministic multi-start search protocol over $\text{SE}(3)$ prior to continuous optimization [@jenkinson2001global; @avants2011reproducible]:
...
In the Symmetric Normalization (SyN) architecture, velocity updates $\delta_l$ and $\delta_r$ are computed independently from similarity gradients at the virtual midpoint $\Omega_{1/2}$ [@avants2008symmetric; @miller2002metrics]. Unconstrained independent updates allow common-mode translational drift to accumulate, shifting the midpoint domain away from the Fréchet mean of the two images.
...
### 2.8 Unbiased Antithetic Bootstrapped Gradient Estimation & The Multi-Scale Deformation Energy Dynamics

#### 1. Discrete Discretization Aliasing vs. Pointwise Gradient Noise
...
![Figure 14: Multi-Scale Deformation Energy and Gradient Dynamics on the 2D `r16` $\to$ `r64` benchmark across iterations and smoothing regimes. **Row 1 ($\sigma_{\text{flow}}=0$, Iteration 1)**: ANTs C++ SyN exhibits high-frequency speckled gradient noise throughout the parenchyma ($E_{\text{harm}} = 6.47 \times 10^{-4}$), whereas autograd concentrates smooth forces along boundaries ($E_{\text{harm}} = 4.06 \times 10^{-4}$ and $3.62 \times 10^{-4}$). **Row 2 ($\sigma_{\text{flow}}=0$, Iteration 2)**: Noise accumulates rapidly in ANTs ($E_{\text{harm}} = 2.11 \times 10^{-3}$) compared to syntx ($1.28 \times 10^{-3}$). **Row 3 ($\sigma_{\text{flow}}=3.0$, Iteration 1)**: Gaussian smoothing spreads ANTs noise over wider radii ($E_{\text{harm}} = 1.00 \times 10^{-3}$), while autograd maintains focused boundary flows ($2.71 \times 10^{-4}$). **Row 4 ($\sigma_{\text{flow}}=3.0$, Iteration 2)**: Smooth accumulation demonstrates that autograd generates coherent, non-stagnating anatomical motion.](figures/fig14_antithetic_bootstrapping_r16_r64.png){width=100%}

#### 3. Resolving the Deformation Energy Paradox: Noise vs. Coherent Motion
A fundamental question arises when comparing deformation energy across registration regimes: *Why does ANTs exhibit higher energy in raw/early iterations, yet lower energy across long multi-resolution schedules?*

The multi-row empirical investigation in Figure 14 resolves this mechanism:
- **Raw Step Dynamics (Rows 1–2)**: At single-step resolution with zero smoothing ($\sigma_{\text{flow}}=0$), ANTs C++ SyN produces significantly higher harmonic energy ($E_{\text{harm}} = 6.47 \times 10^{-4}$) and bending energy ($\text{Bnd} = 7.47 \times 10^{-4}$) than `syntx` ($E_{\text{harm}} = 4.06 \times 10^{-4}$, $\text{Bnd} = 4.91 \times 10^{-4}$). This occurs because ANTs pseudo-derivatives generate incoherent, spiky micro-vectors pointing in conflicting directions across neighboring voxels.
- **Smoothing & Early Accumulation (Rows 3–4)**: Under standard Gaussian smoothing ($\sigma_{\text{flow}}=3.0\text{ mm}$), ANTs raw speckles are blurred into wide non-specific displacement domes ($E_{\text{harm}} = 3.64 \times 10^{-3}$, $\bar{u}=0.059\text{ mm}$), whereas `syntx` autograd retains compact, boundary-confined flow trajectories ($E_{\text{harm}} = 1.11 \times 10^{-3}$, $\bar{u}=0.030\text{ mm}$).
- **Long-Term Multi-Resolution Trajectories (210+ Iterations)**: Over hundreds of iterations across multi-scale pyramids, ANTs' incoherent micro-vectors partially cancel each other out or stagnate in local minima, capping total cumulative displacement. In contrast, `syntx` autograd provides **strictly coherent, persistent descent directions along true anatomical boundaries**, driving deep cortical alignment (achieving $+1.5\%\text{ to }+4.5\%$ higher DICE over ANTs) and accumulating higher net displacement at convergence.
- **The Antithetic Advantage**: By destructively eliminating discrete boundary aliasing ($\mathbb{E}[\boldsymbol{\delta}]=\mathbf{0}$), Antithetic Bootstrapping prevents high-frequency sulcal shearing without sacrificing coherent boundary descent, delivering **`0.00000%` folding reliability** across the entire cohort.
...
Standard first- and second-moment adaptive optimizers (such as Adam [@kingma2014adam; @reddi2019convergence]) compute coordinate-wise parameter updates:
...
To reconcile the adaptive learning rate advantages of moment-based optimization with the strict smoothness requirements of diffeomorphic flows, we formulate optimization on the Sobolev Hilbert space $H^s(\Omega; \mathbb{R}^d)$ equipped with the inner product [@neuberger2010sobolev; @sundaramoorthi2007sobolev]:
...
The Riesz representation theorem establishes that the Riemannian gradient $\nabla_{H^s} \mathcal{E}$ with respect to the $H^s$ metric is obtained by applying the Green's operator $\mathcal{G}_{\text{Sobolev}} = (I - \alpha \Delta)^{-s}$ to the standard $L^2$ functional gradient [@trouve1998diffeomorphisms; @neuberger2010sobolev]:
...
Registration performance is evaluated across **90 3D T1-weighted brain volume pairs** (40 intra-subject longitudinal test-retest pairs and 50 inter-subject cross-site pairs) sampled from the standardized Mindboggle-101 cohort [@klein2012101; @klein2017mindboggle; @klein2009evaluation].

#### 1. Evaluation Metrics
Registration quality is benchmarked using utility-computed quantitative metrics against ground-truth Desikan-Killiany-Tourville (DKT31) cortical labels [@desikan2006automated; @fischl2002whole; @tustison2014large]:
...
Tensor-accelerated primitives in PyTorch and JAX [@paszke2019pytorch; @jax2018github; @lowekamp2013design] yield substantial throughput gains across all compute architectures:
...
2. **4-Panel Diagnostic Metrology**:
   ...
   - **High-Contrast Boundary Overlap**: Structural edge contour overlay extracted via Canny filter operators [@canny1986computational].
...
3. **Avants, B. B., Tustison, N. J., Song, G., Cook, P. A., Klein, A., & Gee, J. C. (2011).** A reproducible evaluation of ANTs similarity metrics in brain image registration. *NeuroImage*, 54(3), 2033–2044. doi:[10.1016/j.neuroimage.2010.09.025](https://doi.org/10.1016/j.neuroimage.2010.09.025).
4. **Balakrishnan, G., Zhao, A., Sabuncu, M. R., Guttag, J., & Dalca, A. V. (2019).** VoxelMorph: A learning framework for deformable medical image registration. *IEEE Transactions on Medical Imaging*, 38(8), 1788–1800. doi:[10.1109/TMI.2019.2897538](https://doi.org/10.1109/TMI.2019.2897538).
5. **Beg, M. F., Miller, M. I., Trouvé, A., & Younes, L. (2005).** Computing large deformation metric mappings via geodesic flows of diffeomorphisms. *International Journal of Computer Vision*, 61(2), 139–157. doi:[10.1023/B:VISI.0000045752.48366.19](https://doi.org/10.1023/B:VISI.0000045752.48366.19).
...
7. **Canny, J. (1986).** A computational approach to edge detection. *IEEE Transactions on Pattern Analysis and Machine Intelligence*, PAMI-8(6), 679–698. doi:[10.1109/TPAMI.1986.4767851](https://doi.org/10.1109/TPAMI.1986.4767851).
8. **Chirikjian, G. S. (2011).** *Stochastic Models, Information Theory, and Lie Groups, Volume 2: Analytic Methods and Modern Applications*. Applied and Numerical Harmonic Analysis, Birkhäuser, Boston, MA. doi:[10.1007/978-0-8176-4944-9](https://doi.org/10.1007/978-0-8176-4944-9).
9. **Christensen, G. E., Rabbitt, R. D., & Miller, M. I. (1996).** Deformable templates using large deformation kinematics. *IEEE Transactions on Image Processing*, 5(10), 1435–1447. doi:[10.1109/83.536892](https://doi.org/10.1109/83.536892).
10. **Cooley, J. W., & Tukey, J. W. (1965).** An algorithm for the machine calculation of complex Fourier series. *Mathematics of Computation*, 19(90), 297–301. doi:[10.1090/S0025-5718-1965-0178586-1](https://doi.org/10.1090/S0025-5718-1965-0178586-1).
11. **Courant, R., Friedrichs, K., & Lewy, H. (1928).** Über die partiellen Differenzengleichungen der mathematischen Physik. *Mathematische Annalen*, 100(1), 32–74. doi:[10.1007/BF01448839](https://doi.org/10.1007/BF01448839).
